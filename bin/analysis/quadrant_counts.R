...
#fil=read.csv("/users/cn/sjin/projects/homoplasy/nf_homoplasty/data/data_14tree_above1000/delta_viceversa_data.csv",header=TRUE,sep=" ",row.names=NULL)
...
count_quadrants=function(df,metric,score)
{
    ...
    pos=which(get(metric,df)>0 & get(score,df)>=0 & abs(get(metric,df))>0.001 & abs(get(score,df))>0.001)
    q=c(q,length(pos))
...
for (metric in metrics[2:length(metrics)])
{
    ...
    positive=q1+q3
    negative=q2+q4
    df2=data.frame(family=rep(fams,length(aligners)),aligner=al,q1=q1,q2=q2,q3=q3,q4=q4,positive=positive,negative=negative,diff=positive-negative,np_ratio=negative/positive,nperc=negative/(q1+q2+q3+q4))
    n=n+1
    countList[[n]]=df2
}
names(countList)=metrics[2:length(metrics)]
