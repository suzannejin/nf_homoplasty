...
cwd=$(pwd)
echo "family;mode;bucket;aligner;tree;tc;homo;whomo;whomo2;len;ngap;ngap2"
        ...
        do
            [[ $mode1 == "reg_align" ]] && mode2="dpa_align"
                ...
                do
                        ...
                        do
                                f=${foldname[$n]}
                                cd ${cwd}/results_fullTree_${f}_bucket${nSeq}

                                for tree_method in ${tree[@]} ## loop each tree
                                do
                                        printf "${family};${mode1};${nSeq};${align_method};${tree_method};"

                                        if [[ -s ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.aln ]]; then
                                        cat ./individual_scores/${family}.${mode2}.${nSeq}.${align_method}.${tree_method}.tc \
                                        ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.homo \
                                        ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.w_homo \
                                        ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.w_homo2 \
                                        ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.len \
                                        ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.ngap \
                                        ./alignments/${family}.dpa_${nSeq}.${align_method}.with.${tree_method}.tree.ngap2 \
                                        | tr -d "l"| tr '\t' ';'| tr "\n" ";" | tr -d "[:space:]" | sed 's/.\{1\}$//'
                                        #| tr -d "l" | tr '\t' ';'| tr -d "[:space:]"     
                                        # cat ./individual_scores/${family}.${flavour}.${nSeq}.${align_method}.with.${tree_method}.tree.tc \
                                        # ./homoplasy/${family}.${flavour}.${nSeq}.${align_method}.with.${tree_method}.tree.homo \
                                        # ./homoplasy/${family}.${flavour}.${nSeq}.${align_method}.with.${tree_method}.tree.w_homo \
                                        # ./homoplasy/${family}.${flavour}.${nSeq}.${align_method}.with.${tree_method}.tree.w_homo2 \
